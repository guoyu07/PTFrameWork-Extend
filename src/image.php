...
class Image {
     ...
     * @return mixed
     */
    public function text($text, $font=null, $size = 20, $color = '#ff0000', $locate = Image::IMAGE_WATER_SOUTHEAST, $margin = '', $offset = 0, $angle = 0) {
        //资源检测
        //if (!is_file($font)) return false;
        if ($margin === '') $margin = ceil($size / 2);
        //获取文字信息
        if(is_null($font)) $font=__DIR__.'/data/ptcms.otf';
